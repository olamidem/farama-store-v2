...
} from "../validation/stockAdjustment.schema";
...
export const StockAdjustmentModal = ({
  ...
  const { data: productUnits = [], isLoading: isLoadingUnits } =
    useProductUnits(selectedProductId);
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-1.5 text-left">
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
              ...
              {isLoadingUnits && (
                <span className="text-[9px] text-slate-400 lowercase animate-pulse">
                  Loading units...
                </span>
              )}
            </label>
                ...
                productUnits.map(
                  (pu: {
                    ...
                    <option key={pu.id} value={pu.id}>
                      {pu.unit?.name || "Piece"} ({pu.unit?.symbol || "pcs"}) -
                      Conversion: {pu.conversion_factor}x
                    </option>
                  ),
                )
              )}
            ...
            <div className="space-y-1.5 text-left">
              ...
              <div className="flex border border-slate-200 rounded-xl overflow-hidden p-1 bg-slate-50">
                <button
                  type="button"
                  disabled={
                    watchedType !== "ADJUSTMENT" && watchedType !== "TRANSFER"
                  }
