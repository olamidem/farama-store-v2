...
interface ProductRestoreSummaryProps {
  product: Product;
  categories: Category[];
}

const ProductRestoreSummary = ({
  product,
  categories,
}: ProductRestoreSummaryProps) => {
  ...
  return (
    <div className="space-y-5 text-xs font-sans">
      ...
      <div className="bg-emerald-50 border border-emerald-500 rounded-xl p-4 text-black text-xs flex gap-2.5 items-start">
        <Info className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
        <div className="space-y-1">
          <p className="font-bold text-emerald-900">Catalog Restoration Rule</p>
          <p className="leading-relaxed">
            You are preparing to restore a deactivated item back into your
            active inventory database. Please review its stock levels and
            pricing rules to ensure it resumes sales seamlessly without tracking
            errors.
          </p>
        </div>
      </div>
...
export default ProductRestoreSummary;
